...
def load_bfix_ohlc(filepath: Path) -> pd.DataFrame:
    # Match the BFIX workbook layout used by the other FX strategy scripts.
...
def normalize_ohlc_columns(df: pd.DataFrame) -> pd.DataFrame:
    # Allow the reusable entry point to accept standard Open/High/Low/Close naming.
    rename_map = {
        "Open": "PX_OPEN",
        "High": "PX_HIGH",
        "Low": "PX_LOW",
...
def rolling_adf_pvalues(series: pd.Series, window: int) -> pd.Series:
    # Rolling ADF remains loop-based because each window runs a regression test.
...
def build_regime_latch(p_values: pd.Series, threshold: float, latch_days: int) -> pd.Series:
    # A qualifying p-value activates the regime for the trigger day and the next
    # few sessions; another trigger while active resets the countdown.
...
def compute_atr(df: pd.DataFrame, window: int) -> pd.Series:
    # True range handles both intraday range and overnight gaps.
...
def compute_rsi(series: pd.Series, window: int) -> pd.Series:
    # Connors RSI-2 here is the classic 2-period RSI built from simple rolling means.
    delta = series.diff()
    ...
    # If both gain and loss are zero, price did not move across the window.
    rsi = rsi.where(~((avg_gain == 0) & (avg_loss == 0)), 50.0)
    return rsi


def prepare_features(df: pd.DataFrame) -> pd.DataFrame:
    # Indicator construction is vectorized; the rolling ADF loop is the only
    # unavoidable iterative component.
    data = normalize_ohlc_columns(df)
    close = data["PX_LAST"]

    data["RSI_2"] = compute_rsi(close, RSI_WINDOW)
    data["SMA_5"] = close.rolling(window=SMA_WINDOW, min_periods=SMA_WINDOW).mean()
    ...
    prev_sma = data["SMA_5"].shift(1)
    data["Long_Entry_Signal"] = data["Regime_Active"] & data["RSI_2"].le(RSI_LONG_THRESHOLD)
    data["Short_Entry_Signal"] = data["Regime_Active"] & data["RSI_2"].ge(RSI_SHORT_THRESHOLD)
    data["Long_Target_Signal"] = prev_close.le(prev_sma) & close.gt(data["SMA_5"])
    data["Short_Target_Signal"] = prev_close.ge(prev_sma) & close.lt(data["SMA_5"])
    return data
...
def simulate_strategy(
    ...
    # The trade engine stores entry ATR so stop placement stays fixed after entry.
    data = df.copy()
    data["Position"] = 0
    data["Units"] = 0.0
    ...
    for idx, row in data.iterrows():
        ...
        if position != 0 and pd.notna(previous_close):
            # Mark to market open positions so the equity curve reflects day-by-day risk.
        ...
        if position == 1:
            ...
            if hit_stop or hit_target:
                # Stops execute at the stop level; profit exits execute at the close of
                # the bar that crosses the target SMA.
        ...
        if position == 0 and pd.notna(row["ATR"]):
            # Enter at the close once the RSI-2 exhaustion condition is present.
...
def plot_equity_curve(result_df: pd.DataFrame, pair: str, output_path: Path) -> None:
    ...
    ax.set_title(f"{pair} ADF RSI-2 Mean-Reversion Equity Curve")
    ax.set_xlabel("Date")
...
def main() -> None:
    base_dir = Path(__file__).resolve().parent
    output_dir = base_dir / "backtest_outputs_rsi2"
    ...
    for config in PAIR_CONFIGS:
        ...
        print(f"\n=== {config.pair} ADF RSI-2 Mean-Reversion Summary ===")
